export class CreditCardManager {
  ...
  initializePage() {
    ...
    this.renderCards(this.currentPage);
  }
  ...
  renderCardsPage() {
    ...
    cardsContainer.innerHTML = `
      ...
      <button id="connect-open-finance" class="open-finance-btn">Conectar ao Open Finance</button>
  ...
  setupCardActions() {
    ...
    document.getElementById('cardDetailsForm').addEventListener('submit', (e) => {
      e.preventDefault();
      this.addNewCard();
    ...
    window.addEventListener('click', (e) => {
      if (e.target === document.getElementById('cardDetailsModal')) {
        document.getElementById('cardDetailsModal').style.display = 'none';
      }
    });
  }
  ...
  renderCards(page) {
    ...
    document.getElementById('page-info').textContent = `Página ${page} de ${Math.ceil(this.cards.length / this.cardsPerPage)}`;
  ...
  async connectOpenFinance() {
    try {
      const response = await fetch("https://api.openfinance.com.br/dados");
      const data = await response.json();
      this.cards = data;
      localStorage.setItem('creditCards', JSON.stringify(this.cards));
      this.renderCards(this.currentPage);
      alert("Dados do Open Finance carregados com sucesso!");
    } catch (error) {
      console.error("Erro ao conectar ao Open Finance:", error);
      alert("Erro ao conectar ao Open Finance.");
    }
  }
}
